...
market dynamics is examined by injecting shocks to the order book. Finally
the short comings of zero intelligent traders in terms of arbitrage opportunities
are inspected with a settings of two identical markets. In all of the experiments
all of the traders are given the opportunity make one trading decision per session,
...
is studied using stylized facts. In order to have represenative data set to 
...
sessions are slightly lighter than normal distribution. To summarize, the model do have fatter tails than normal distribution
...
traders do not produce efficient results, the third experiment is to study the arbitrage in 
zero-intelligent markets. The simulation settings is similar to the other experiments except instead of one
market there are two markets trading same asset with same currency.
